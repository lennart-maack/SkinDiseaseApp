...
class Predictor():
    ...
    def __init__(self, img_save_path, gt_path):

        self.mean = [0.485, 0.456, 0.406]
        self.std = [0.229, 0.224, 0.255]
        self.input_path_predict_img = img_save_path
        self.groundtruth_path = gt_path
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = nnModel.SEResnext50_32x4d(pretrained=None)
        self.model_path = os.path.join("models", "model_8.bin")
    ...
    def __resize_image(self):
        ...
        img = Image.open(os.path.join(self.input_path_predict_img, image_names[0][0])).convert('RGB')
        img = img.resize(
            (512, 512), resample=Image.BILINEAR
        )
        img.save(os.path.join(self.input_path_predict_img, image_names[0][0]), "PNG")
